...
for i=1:Wd
    Xmax(1,72*(i-1)+1:72*(i-1)+72)=[upx(D+i).*P_wind(1,:),upx(D+i).*P_wind(2,:),upx(D+i).*P_wind(3,:)];
    Xmin=gama*Xmax;
end
...
%外部归档集初始化****************************************
archive=[]; 
%pareto临时最优断面在种群更新完成之后，MOPSO进行了三轮筛选。
...
while(iterations<=maxIterations)
    ...
    for b=1:n
        [fx_temp1,fx_temp2,Cy,Cg,Cq,Cdan,Closs,Vp,E_soc,P_st,Inv1]=low_obj(x(b,:),upx);
        disp(['第',num2str(b),'个粒子的low_obj结束']);%调试用
        fx(b,:)=[fx_temp1,fx_temp2];
        P(b,:)=[E_soc P_st Inv1 fx_temp1 fx_temp2 zeros(1,1)]; %保存运行信息
        Val(b,:)=[Cy Cg Cq Cdan Closs Vp fx_temp1 fx_temp2 zeros(1,1)];
        %P_Li(b,:)=P_lizi;
    end
...
%-----外部归档集初始化；外部归档集以Stem1的形式保存粒子信息（位置；适应度；拥挤距离信息）------%
    ...
    if iterations==maxIterations
        ...
        %粒子的速度更新
        v=updata_v(wmax,wmin,iterations,maxIterations,n,x,v,px,gx,Vmax,d);
        %粒子的位置更新
        x=x+v;       
